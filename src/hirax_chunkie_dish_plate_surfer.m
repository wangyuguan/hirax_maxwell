function [S,parts] = hirax_chunkie_dish_plate_surfer(thickness,opts)
%HIRAX_CHUNKIE_DISH_PLATE_SURFER Smooth HIRAX-shaped thin plate.
%
...
% the nominal dish radius.  No saved msh, rquad, or surfer mesh is loaded.
...
parts.design = design;
parts.dish_radius = design.dish_scale;
...
dish_center = [0;0.5*(bottom_left(2)+y0+ellipse_radius_y)];
dish_scale = ellipse_radius_x;
...
outline.raw_vertices = raw_vertices-dish_center;
[outline.panel_starts,outline.panel_start_derivatives] = ...
    chunkie_panel_starts(outline);

design = struct();
design.bottom_left = bottom_left-dish_center;
design.bottom_right = bottom_right-dish_center;
design.right_circle_start = right_circle_start-dish_center;
design.right_ellipse_join = right_ellipse_join-dish_center;
design.left_ellipse_join = left_ellipse_join-dish_center;
design.left_circle_end = left_circle_end-dish_center;
design.dish_center_raw = dish_center;
design.dish_scale = dish_scale;
